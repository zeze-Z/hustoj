...
static  $SMTP_PORT=465;                           //SMTP服务器端口，通常是25，有的服务器支持80（阿里云）、465(网易)、587（QQ）以适应不同的网络防火墙配置
static  $SMTP_USER="aioj.top@qq.com";      //SMTP服务器的用户名（通常就是发件人的邮箱地址）, 这里修改后视为邮件配置生效，若配置不当可能导致部分页面超时。
static  $SMTP_PASS="smtp_acc_pswd";       //由邮箱系统生成的口令 (SMTP服务器的密码)
...
static 	$OJ_REG_SPEED=1 ; //限制每小时同ip注册个数，0不限制
...
static 	$OJ_BG="";  //站点背景壁纸，留空则不请求背景图。例如配置：$OJ_BG="http://cdn.hustoj.com/upload/bg/bing".date('H').".jpg"; 每个整点更换壁纸，例如准备bing[00~23].jpg在image目录。如果使用变量，需要移除开头的static修饰词。
...
static  $OJ_GUEST=true; //允许游客以Guest身份浏览
...
static  $OJ_AINO=false; //开启AI防作弊提示次
...
static  $OJ_BEIAN="蜀ICP备2026007934号-1";  // 如果有备案号，填写备案号
static  $OJ_CUSTOMER_QQ="2326077585";  // 客服QQ号，页脚/登录/注册/教师引导页统一读取，修改此处即可全局生效
...
static $OJ_COURSE_SHARE_SECRET = 'reiga209rhjer98ha9';