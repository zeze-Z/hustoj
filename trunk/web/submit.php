...
if (~$OJ_LANGMASK & (1 << $language)) {
    ...
    if (!isset($pid)) {
        $sql = "INSERT INTO solution(problem_id,user_id,nick,in_date,language,ip,code_length,result) VALUES(?,?,?,NOW(),?,?,?,0)";
        $insert_id = pdo_query($sql, $id, $user_id, $nick, $language, $ip, $len);
    } else {
        $sql = "INSERT INTO solution(problem_id,user_id,nick,in_date,language,ip,code_length,contest_id,num,result) VALUES(?,?,?,NOW(),?,?,?,?,?,0)";
