...
if ($order_id <= 0) {
    echo "<script>alert('无效请求'); window.location.href='course_order.php';</script>";
    exit(1);
}
...
if (empty($order_result)) {
    echo "<script>alert('订单不存在'); window.location.href='course_order.php';</script>";
    exit(1);
}
...
if ($order['pay_status'] != 1) {
    echo "<script>alert('订单未支付，无法发送邮件'); window.location.href='course_order.php';</script>";
    exit(1);
}
...
if (send_course_mail($order['email'], $course_data)) {
    ...
    echo "<script>alert('邮件发送成功'); window.location.href='course_order.php';</script>";
} else {
    // 更新邮件发送失败状态
    pdo_query(
    ...
    echo "<script>alert('邮件发送失败，请检查SMTP配置'); window.location.href='course_order.php';</script>";
}
