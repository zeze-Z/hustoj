...
1. 登录 OJ 网站（aioj.top）
2. 进入「更多」页面，找到「离线游戏」
3. 选择套餐并积分兑换，填写**学校名称** + **机房名称**：
   - **全套17款**：兑换后解锁离线包内全部游戏（50 积分，价格以网站商品配置为准）
   - **🎉 任选3款促销包**：在弹窗的17款游戏中勾选恰好3款（29 积分，价格以网站商品配置为准）；授权码仅可打开所选3款，游戏大厅只显示已授权游戏，直接打开其余游戏页面会被拦截
4. 兑换成功后获得 JSON 授权码，同时通过网盘链接下载离线游戏包

> 促销包与全套使用同一个通用离线包，激活方式完全相同；同一学校/机房两套授权可分别兑换，激活哪份授权码就生效哪份（激活全套即解锁全部17款）。
...
- 有效期为1年
- 支持离线验证（验签在本地浏览器完成）
- 授权码 JSON 可选携带 `games` 白名单（任选3款促销包）；签名串为 `学校|机房|到期日|id1,id2,id3`（id 排序）。无 `games` 字段的旧授权码按 `学校|机房|到期日` 验签并视为全套，**历史授权码全部继续有效**
- 手动生成促销包授权：`generate_license.py --school "XX小学" --room "机房1" --expire 2027-09-01 --games snake,math_game,minesweeper`（恰好3个合法 id）
