...
//                      $contest_ok = false;
    ...
    if (!isset($_SESSION[$OJ_NAME . '_' . 'administrator']) && $unixnow < $start_time) {
        ...
        if (!$contest_ok) {
            ...
            $view_errors .= "<p style='margin-top:20px;'>比赛将在 $view_start_time 开始，请稍候</p>";
            $view_errors .= "</center>";
            $view_errors .= "<br><br>";
...
if (!$contest_ok) {
    ...
    // 根据不同的失败原因显示不同的提示
    if ($view_private && !isset($_SESSION[$OJ_NAME . '_' . 'c' . $cid])) {
        // 因为是私有竞赛且没有权限
        $view_errors .= "<p class='text-danger'>这是一个私有竞赛，需要密码或权限才能访问</p>";
    } else {
        // 其他原因（IP限制等）
        if ($view_description) {
            $view_errors .= "<p class='text-danger'>$view_description</p>";
        } else {
            $view_errors .= "<p class='text-danger'>$MSG_PRIVATE_WARNING</p>";
        }
    }
    ...
    if (!empty($row['password'])) {
        ...
        $view_errors .= "<input class=input-mini type=password name=password value='' placeholder='请输入竞赛密码'>";
